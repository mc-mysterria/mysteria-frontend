...
function createViteConfig({mode}: ConfigEnv): UserConfig {
    ...
    const plugins: PluginOption[] = [
        ...
        vueDevTools(),
        vercel(),
        // No sitemap/robots plugin here on purpose. vite-plugin-sitemap had no route
        // list to work from (this is a SPA with no file-based routing), so it emitted
        // a sitemap containing only "/" plus a JS chunk URL, and it overwrote the
        // carefully-scoped public/robots.txt with a bare "Allow: /". robots.txt is
        // now served straight from public/, and scripts/build-sitemap.mjs generates
        // the real sitemap after the build.
    ]
