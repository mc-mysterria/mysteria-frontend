...
**Files:**
- `src/stores/dailyBonus.ts` — Pinia store; fetches `/api/bonus/daily/status`, picks a random page from ['home','shop','rules','profile'], `claim()` calls POST `/api/bonus/daily/claim` then refreshes balance
...
- Added to: HomeView, ProfileView, RulesView, ShopView
