...
import type {CreateNewsData, NewsArticle, NewsPreview, Page, UpdateNewsData} from "@/types/news";
...
class NewsAPI extends BaseCRUD<NewsArticle, CreateNewsData, UpdateNewsData, Record<string, never>> {
    ...
    async getPinned(language: 'en' | 'uk'): Promise<APIResponse<NewsPreview[]>> {
        return this.request<NewsPreview[]>("GET", `/${language}/pinned`);
    }
